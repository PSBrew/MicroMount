...
#  MM_FORCE_REBUILD_IMAGE=1 ./build.sh

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
DOCKER_IMAGE="${MM_DOCKER_IMAGE:-ubuntu:24.04}"
APT_PACKAGES=(
  autoconf
  automake
  build-essential
...
Optional env vars:
  MM_FORCE_REBUILD_IMAGE=1   Rebuild the cached Docker image before build
EOF
}
...
DEFAULT_LOCAL_BUILD_IMAGE="mm-build-cache:$(sanitize_image_key "$DOCKER_IMAGE")"
LOCAL_BUILD_IMAGE="$DEFAULT_LOCAL_BUILD_IMAGE"
FORCE_REBUILD_IMAGE="${MM_FORCE_REBUILD_IMAGE:-0}"
...
RUN useradd -m -s /bin/bash mmbuilder
...
RUN chown -R mmbuilder:mmbuilder /opt/pacbrew-repo /opt/ps5-sdk
RUN ln -sf /proc/self/mounts /etc/mtab
RUN su - mmbuilder -c 'cd /opt/pacbrew-repo/sdk && makepkg -c -f'
RUN pacman --noconfirm -U /opt/pacbrew-repo/sdk/ps5-payload-*.pkg.tar.gz
RUN su - mmbuilder -c 'cd /opt/pacbrew-repo/sqlite && makepkg -c -f'
